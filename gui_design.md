...
- 入力内容: イベント基本情報 + 詳細設定（ジャンル、参加条件、備考など）
...
- 必須項目
  ...
  - `genres`, `participation_conditions`, `participation_method`, `remarks` などその他の項目は任意入力としつつ、適宜入力を推奨する
  ...
  - 構成:
    ...
    - 「入力内容」タブ:
      ...
      - 詳細設定:
        ...
        - 複数行テキスト: `QTextEdit` を利用（イベント内容 / 参加条件 / 参加方法 / 備考）
        - メールアドレス記録: `QCheckBox`
    - 「実行 / ログ」タブ:
      - 上部に説明テキスト（`QLabel`）
